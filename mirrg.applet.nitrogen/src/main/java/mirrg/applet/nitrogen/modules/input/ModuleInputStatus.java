...
public class ModuleInputStatus extends Module
{
	...
	public ModuleInputStatus(AppletNitrogen applet, ModuleInputEvent moduleInputEvent)
	{
		...
		applet.getEventManager().register(NitrogenEventMouse.Pressed.class, event -> {
			mousePressed(
				event.mouseEvent.getX(),
				event.mouseEvent.getY(),
				event.mouseEvent.getButton());
		});
		applet.getEventManager().register(NitrogenEventMouse.Released.class, event -> {
			mouseReleased(
				event.mouseEvent.getX(),
				event.mouseEvent.getY(),
				event.mouseEvent.getButton());
		});
		applet.getEventManager().register(NitrogenEventMouseMotion.class, event -> {
			mouseMotion(
				event.mouseEvent.getX(),
				event.mouseEvent.getY());
		});
		applet.getEventManager().register(NitrogenEventMouseWheel.Moved.class, event -> {
			mouseWheelMoved(event.mouseWheelEvent.getWheelRotation());
		});
		applet.getEventManager().register(NitrogenEventKey.Pressed.class, event -> {
			keyPressed(event.keyEvent.getKeyCode());
		});
		applet.getEventManager().register(NitrogenEventKey.Released.class, event -> {
